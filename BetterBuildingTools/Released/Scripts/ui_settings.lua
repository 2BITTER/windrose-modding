    ...
    local SectionDefs = {
        ...
        },
        {
            ...
            items = {
                ...
                { key = "placement_no_bell_shore",    label = "No Shore Required",        type = "bool", hint = "Bells + wharfs anywhere" },
        ...
        if Settings.ViewMode == "headers" then
            for i = 1, #SectionDefs do
                ...
                if lbl and lbl:IsValid() then
                    ...
                    if sel then
                        lbl:SetColorAndOpacity(H.COLOR_SELECTED)
                    elseif SectionDefs[i].cheat then
                        lbl:SetColorAndOpacity(H.COLOR_ORANGE)
                    else
                        lbl:SetColorAndOpacity(H.COLOR_WHITE)
                    end
                end
                if arr and arr:IsValid() then
                    arr:SetColorAndOpacity(sel and H.COLOR_SELECTED or H.COLOR_DIM)
                end
            end
            ...
            for i, item in ipairs(sec.items) do
                ...
                if lbl and lbl:IsValid() then
                    ...
                    if item.type == "status" then
                        lbl:SetColorAndOpacity(sel and H.COLOR_SELECTED or H.COLOR_DIM)
                ...
                if vlbl and vlbl:IsValid() then
                    if item.type == "status" then
                        ...
                        vlbl:SetColorAndOpacity(H.COLOR_DIM)
                    elseif item.type == "bool" then
                        ...
                        vlbl:SetColorAndOpacity(val and H.COLOR_VALUE_ON or H.COLOR_VALUE_OFF)
                    elseif item.type == "int" then
                        local val = item.key and BBT_GetConfig and BBT_GetConfig(item.key)
                        vlbl:SetText(FText(tostring(val or 0)))
                        vlbl:SetColorAndOpacity(H.COLOR_VALUE_NUM)
                    end
                end
            end
